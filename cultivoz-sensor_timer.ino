...
int valorUmidadeSoloMedio = 50; // alteração de teste para funcionar já parametrizando um valor em %                  ///
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void setup() {
  ...
  mudaEstadoTorneira(RELEVASOPEQUENO,DESLIGADO); // Define que o sistema inicia com o a torneira do vaso pequeno deligada - altere para LIGADO se quiser o contrário
...
void loop() {
 ...
 // lerSensorSolo(SensorSoloGrande, valorUmidadeSoloGrande, ReleVasoGrande);
  ...
  mudaEstadoTorneira(RELEVASOMEDIO,checkSensor(SENSORSOLOMEDIO, valorUmidadeSoloMedio));

  if ( now() == (TimerVasoPequeno+IntervaloVasoPequeno)) { 
     mudaEstadoTorneira(RELEVASOPEQUENO,!(digitalRead(RELEVASOPEQUENO)));
     TimerVasoPequeno = now();
  }
}

void mudaEstadoTorneira(int parametroReleTorneira, boolean Estado){ // altera o valor do relé escolhido para o parametro (ligado ou desligado) determinado 

  digitalWrite(parametroReleTorneira, Estado); }
...
boolean checkSensor(int parametroSensorSolo, int parametroValorUmidade){ // checa se o sensor escolhido apresenta um indice de umidade menor que o valor definido em %
   ...
   int valorSensorSolo = 100-(map(analogRead(parametroSensorSolo),0,1023,0,100)); 
   return (valorSensorSolo <= parametroValorUmidade);
}
